...
export class ParkingHistoryPage implements OnInit {
  ...
  ionViewDidEnter() {
    ...
    this.loadingPresent();
    this.http.get(environment.base_api + 'parkedCarsHistory/' + this.enforce_id).subscribe((res) => {
      this.weatherResult = true;
      this.parking = res;
      this.loadingDismiss();
      if(res==0){
        this.error="Cars not found.";
      }else{
    ...
    buttons: [
      {
        ...
        text: 'Okay',
        handler: () => {
          this.http.get(environment.base_api + 'deleteCars/' + id).subscribe((res) => {
            if(res==1){
              this.presentToast('Car deleted.');
